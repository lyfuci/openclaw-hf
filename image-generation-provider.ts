...
async function callHuggingFace(params: {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `hf image generation failed: ${describeError(response.status, errBody)}`,
    );
  }
...
function buildAsset(params: {
  ...
  return {
    ...
    fileName: `hf-${Date.now()}.${extension}`,
    metadata: {
...
export function buildHfImageGenerationProvider(): ImageGenerationProvider {
  return {
    id: PROVIDER_ID,
    ...
    async generateImage(req: ImageGenerationRequest): Promise<ImageGenerationResult> {
      if (req.inputImages && req.inputImages.length > 0) {
        throw new Error("hf does not support image-to-image edits in this release");
      }
      ...
      if (!apiKey) {
        throw new Error(
          "Hugging Face API key is not configured. Run `openclaw onboard --auth-choice hf-api-key` or set HUGGINGFACE_HUB_TOKEN.",
        );
      }
