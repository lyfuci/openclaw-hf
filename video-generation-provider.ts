...
function isFalAiOnlyModelMessage(modelId: string, providers: string[]): string {
  return [
    `hf video: model "${modelId}" is not available on a HF Pro free route.`,
...
async function postReplicatePrediction(params: {
  ...
  if (!response.ok) {
    throw new Error(
      `hf video: replicate POST failed (${response.status}): ${text || "unknown error"}`,
    );
  }
...
async function pollReplicatePrediction(params: {
  ...
  while (Date.now() < deadline) {
    if (params.signal?.aborted) {
      throw new Error("hf video: request aborted");
    }
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => "");
      throw new Error(
        `hf video: replicate GET failed (${response.status}): ${text || "unknown"}`,
      );
    }
    ...
    if (body.status === "failed" || body.status === "canceled") {
      throw new Error(
        `hf video: replicate prediction ${body.status}: ${body.error ?? "no error message"}`,
      );
    }
  }
  throw new Error(
    `hf video: replicate prediction did not finish within ${params.timeoutMs}ms`,
  );
}
...
async function fetchVideoBytes(url: string): Promise<{ buffer: Buffer; mimeType: string }> {
  ...
  if (!response.ok) {
    throw new Error(
      `hf video: failed to download generated video (${response.status})`,
    );
  }
...
function pickOutputUrl(prediction: ReplicatePrediction): string {
  ...
  throw new Error("hf video: replicate succeeded but no output URL was returned");
}
...
export function buildHfVideoGenerationProvider(): VideoGenerationProvider {
  return {
    id: PROVIDER_ID,
    ...
    async generateVideo(req: VideoGenerationRequest): Promise<VideoGenerationResult> {
      ...
      if (!apiKey) {
        throw new Error(
          "hf video: HF API key not configured. Set HUGGINGFACE_HUB_TOKEN/HF_TOKEN.",
        );
      }
      ...
      try {
        ...
        if (!predictionId) {
          throw new Error("hf video: replicate did not return a prediction id");
        }
