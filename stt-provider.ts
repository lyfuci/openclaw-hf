...
function describeError(status: number, body: WhisperResponse | string): string {
  if (typeof body === "string") {
    return body || `hf STT request failed with status ${status}`;
  }
  if (body.error) {
  ...
  return `hf STT request failed with status ${status}`;
}

function buildModelEndpoint(modelId: string): string {
  const safeId = modelId.trim().replace(/^\/+|\/+$/gu, "");
  if (!safeId) {
    throw new Error("hf STT: model id is empty");
  }
...
async function transcribeAudio(req: AudioTranscriptionRequest): Promise<AudioTranscriptionResult> {
  ...
  if (!apiKey) {
    throw new Error("hf STT: HF API key not provided in transcription request");
  }
  ...
  try {
    ...
    if (typeof body === "string" || typeof body.text !== "string") {
      throw new Error("hf STT response is missing `text` field");
    }
    return {
      text: body.text,
      model: modelId,
...
export const hfMediaUnderstandingProvider: MediaUnderstandingProvider = {
  id: PROVIDER_ID,
  capabilities: ["audio"],
  defaultModels: {
